...
#include "Abilities/Actions/AsyncTaskCooldownChanged.h"
...
UAsyncTaskCooldownChanged* UAsyncTaskCooldownChanged::ListenForCooldownChange(UAbilitySystemComponent* AbilitySystemComponent, FGameplayTagContainer InCooldownTags, bool InUseServerCooldown)
{
	...
	//uses InCooldownTags name because CooldownTags is a member of the class
	...
	if (!IsValid(AbilitySystemComponent) || InCooldownTags.Num() < 1)
	{
		...
		UE_LOG(LogTemp, Error, TEXT("UAsyncTaskCooldownChanged ListenForCooldownChange called with null AbilitySystemComponent or empty CooldownTags"));
		return nullptr;
	}
